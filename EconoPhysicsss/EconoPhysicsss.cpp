...
float Agent_1(int[num_agents][num_goods], float[]);

//information of agents and what they have:goods, cash
int main()
{
	int Goods[num_agents][num_goods]; //creates a 2-d array of agents and number of goods they have
	...
	int calories[num_goods] = { 100, 50, 250, 200, 300 }; // index of calories each good has
...
float Agent_1(int Goods[num_agents][num_goods], float Cash[]) 
{
	int gmax[num_goods] = {10, 5, 20, 30, 5};
	float Q = 1;
	float Q0 = 5;
	float utility_1g = 10;
	...
	for (int a = 0;a <= num_goods; a++) {
		 float placeholder = (Goods[0][a] / 5) / ((Goods[0][a] / 5) + 1);
		 utility_1g = placeholder + (Goods[0][a] / gmax[a]) / ((Goods[0][a] / gmax[a]) + 1);
	}	
